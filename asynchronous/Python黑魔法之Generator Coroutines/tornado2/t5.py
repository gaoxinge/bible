...
class _IOLoop(object):
    ...
    def add_timeout(self, deadline, future):
        self._timeouts.append((deadline, future))
    ...
    def start(self):
        while True:
            ...
            for timeout in due_timeouts:
                self._timeouts.remove(timeout)
                future = timeout[1]
                future.set_result(None)  # in io_loop, io_loop add task2's run to io_loop from sleep's future
...
def sleep(duration):
    ...
    io_loop.add_timeout(time.time() + duration, future)
